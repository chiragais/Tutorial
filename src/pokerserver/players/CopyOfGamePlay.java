...
//	public List<Card> cardArray; // List of all 52 cards
...
//	int intPlayerCounter = 0;
	...
	public CopyOfGamePlay(PlayersManager plraMgr) {
	...
	public void OnStartGame() {
		...
		if (playerManager.getPlayer(curPlayerId).isSmallBlind()) {
		    ...
		    playerManager.getPlayer(curPlayerId).setTotalBalance(curPlayerBalance);
			ChangePlayerTurn();
...
//			
//			
//			executePlayerAction(15, ACTION_CALL);
//		
//		
//		if (!roundComplete) {
//			CheckRoundComplete();
//		}
//		if (curTurnNo > playerManager.getTotalRoomPlayers()) {
//			curTurnNo = 1;
//		}
//		
//		}
...
//			ShowDownAllPlayerCards();
	...
	public ArrayList<Integer> SortWinner(ArrayList<Integer> allRank) {
		...
		for (int i=1; i<=allRanks.length; i++) {
			for(int j=0;j<allRanks.length-i;j++){
				if(AllPlayersRankArray.get(allRanks[j+1]).ordinal()<AllPlayersRankArray.get(allRanks[j]).ordinal())
				{
	...
	public void FindTopWinnerList(ArrayList<Integer> allRanks, int TopRank) {
		...
		WinnerList.clear();
			for (int i = TopRank; i < allRanks.size(); i++) {
				if (AllPlayersRankArray.get(allRanks.get(i)).ordinal() == AllPlayersRankArray.get(allRanks.get(TopRank)).ordinal()) {
	...
	public void CompareWinners() {
		...
		for (int i = 0; i < WinnerList.size(); i++) {
			ListOfCardArray.add( AllPokerHandsList.get(WinnerList.get(i)).getPokerHandArray());
		}
           
		for (int j = 0, k = 0; j < 5; j++, k = 0) {
			
			while (k < ListOfCardArray.size() - 1) {
				if (ListOfCardArray.get(k).get(j).getValue() > ListOfCardArray.get(k + 1).get(j).getValue()) {
					ListOfCardArray.remove(k + 1);
					WinnerList.remove(k + 1);
					k = 0;
					continue;
				} else if (ListOfCardArray.get(k).get(j).getValue() < ListOfCardArray.get(k + 1).get(j).getValue()) {
	...
	public ArrayList<String> getWinnerCards(){
	   return	AllPokerHandsList.get(WinnerList.get(0)).getWinnerHandCardNameList();
	}
	...
	public void DeclareMoneyToFinalWinners() {
		for (int i = 0; i < WinnerList.size(); i++) {
			if (playerManager.getPlayer(WinnerList.get(i)).isPlayrAllIn() && TotalTableAmt>0) {
...
//						curPlayerBalance);
				  playerManager.getPlayer(WinnerList.get(i)).setTotalBalance(curPlayerBalance);
...
//				playerManager.getPlayer(i).setPlayerBetAmount(betText[i]);
	...
	//	curPlayerBalance = Integer.valueOf(playerBalanceText[curPlayerId]);
		curPlayerBalance = playerManager.getPlayer(curPlayerId).getTotalBalance();
	...
	public void NextTurnPreSet() {
		...
		localPlayerBalance = playerManager.getPlayer(curPlayerId).getTotalBalance();
		if (playerManager.getPlayer(curPlayerId).isSmallBlind()) {
			 callAmt=sbBet;
...
// Calculate Pot amount for all (ALL IN ) members
			...
			if(b && allInAmountArray[i]==0){
				...
				for(int j=0;j<nPlayer;j++)
				{
					if(allInBetAmt<betAmtIntArray[j])
					{
						...
						allInAmountArray[i]=allInAmountArray[i]+betAmtIntArray[j];
					}
				}

				allInAmountArray[i]=allInAmountArray[i]+lastRoundTableAmt;
				System.out.println("this is Allin "+i+" = "+allInAmountArray[i]);
			}
	...
	public void OnRoundCompeleted(){
		...
		for (int i = 0; i<nPlayer; i++) {
			...
			{
			int temp=betText[i];
			 System.out.println("total amount is of "+curRound+""+temp);
			TotalTableAmt= TotalTableAmt+temp;
			}
		}
		TotalTableText= TotalTableAmt+"";
        System.out.println("total amount is of "+curRound+""+TotalTableText);
